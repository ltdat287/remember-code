...
class vB_Attachment_Display_Single_vBBlog_BlogEntry extends vB_Attachment_Display_Single
{
	...
	*/
	public function verify_attachment()
	{
		...
		{
			if ($this->registry->userinfo['userid'] != $this->attachmentinfo['userid'] AND !can_moderate_blog('caneditentries'))
			{
				return false;
			}
		}
		else
		{
			...
			if (($this->attachmentinfo['blog_state'] == 'draft' OR $this->attachmentinfo['pending']) AND !is_member_of_blog($this->registry->userinfo, $this->attachmentinfo))
			{
				return false;
			}
...
class vB_Attachment_Display_Multiple_vBBlog_BlogEntry extends vB_Attachment_Display_Multiple
{
	...
	*/
	public function process_attachment_template($bloginfo)
	{
		...
		$bloginfo['filename'] = htmlspecialchars_uni($bloginfo['filename']);
		$bloginfo['counter'] = vb_number_format($bloginfo['counter']);
		$bloginfo['size'] = vb_number_format($bloginfo['size'], 1, true);
		$bloginfo['postdate'] = vbdate($this->registry->options['dateformat'], $bloginfo['b_dateline'], true);
		$bloginfo['posttime'] = vbdate($this->registry->options['timeformat'], $bloginfo['b_dateline']);

		$bloginfo['attachmentextension'] = strtolower(file_extension($bloginfo['filename']));
