...
class vB_Attachment_Display_Multiple_vBForum_Album extends vB_Attachment_Display_Multiple
{
	...
	*/
	public function process_attachment_template($albuminfo, $showthumbs = false)
	{
		...
		$albuminfo['filename'] = htmlspecialchars_uni($albuminfo['filename']);
		$albuminfo['counter'] = vb_number_format($albuminfo['counter']);
		$albuminfo['size'] = vb_number_format($albuminfo['size'], 1, true);

		// $showthumbs???
		$albuminfo['attachmentextension'] = strtolower(file_extension($albuminfo['filename']));
		$show['thumbnail'] = ($albuminfo['hasthumbnail'] == 1 AND $this->registry->options['attachthumbs'] AND $showthumbs);
		$show['inprogress'] = $albuminfo['inprogress'];

		$albuminfo['postdate'] = vbdate($this->registry->options['dateformat'], $albuminfo['dateline'], true);
		$albuminfo['posttime'] = vbdate($this->registry->options['timeformat'], $albuminfo['dateline']);
...
class vB_Attachment_Store_vBForum_Album extends vB_Attachment_Store
{
	...
	*/
	protected function process_upload($upload, $attachment)
	{
		...
		if (!($attachmentid = parent::process_upload($upload, $attachment)))
		{
			$this->uploadcount--;
			return false;
		}
