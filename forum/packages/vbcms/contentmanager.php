	...
	************/
	...
	public static function getNodePanel($divId)
	{
		...
		$result = "<div id=\"$divId\" style=\"position: absolute;
				...
				<div style=\"left:5px;top:2px;position:absolute;text-align:left;\"><strong>" . $vbphrase['section_navigator'] . "</strong></div><br />
...
//				<input type=\"button\" onclick=\"javascript:getSectionList(1);\" value=\"" . $vbphrase['sort_by_position'] . "\" >
		$result .= "</span></div><br>\n
				...
				<div class=\"tcat\" style=\"position:relative;top:9px;left:10px;font-size:14px;
				font-weight:bold;padding:2px;float:left;padding:5px;border-style:solid;border-width:1px 1px 0 1px;
	...
	public static function getCategoryList($sectionid = false, $show_images = true)
	{
		...
		if ($categories['count'])
		{
			...
			$result = ($show_images ? "<img id=\"catchecked_img_0\" style=\"display:none\"
				src=\"../images/misc/subscribed.png\">" : '')
	...
	****************/
	...
	protected static function getLeaves($sectionid)
	{
		...
		if ($sectionid)
		{
			...
			node2.nodeleft AND node2.noderight WHERE node.new != 1 AND node2.nodeid = $sectionid";
		}
		else
		{
	...
	*****/
	...
	public static function showCategories($nodeid, $per_page = 50, $page = 1)
	{
		...
		$result .= "<tr><td>\n\n\n\n\n";
/*		<div width=\"100%\" style=\"text-align:left\"><input type=\"text\" name=\"title_filter\" size=\"20\">
		...
		$result .= "<tr class=\"tcat\">
			<td class=\"feature_management_header\" colspan=\"4\" align=\"left\" style=\"padding:5px;\">"
		...
		foreach ($data as $record)
		{
			...
			$category_name_display = str_repeat('&mdash;' , $record['cat_level'] * 1) . ' ' . vB_Template_Runtime::escapeJS(htmlspecialchars($record['category']));
			...
			$record['categoryid']  . "\"/>
			<td>$sequence_no </td>
			<td align=\"left\"><span align=\"left\">" .	$category_name_display . "</span>
			<div style=\"float:right\">
				...
				<img src=\"../images/cms/add_small.png\" style=\"border-style:none\"
				...
				<a href=\"javascript:showCatEdit('edit',". $cat_edit_node_id. ', ' . $cat_edit_cat_id . ", '"
					.  vB_Template_Runtime::escapeJS(htmlspecialchars($record['category'])) . "', '" .
					vB_Template_Runtime::escapeJS($vbphrase['pick_a_section_below']) . "')\")\">
					<img src=\"../images/cms/edit_small.png\" style=\"border-style:none\" title=\"" .
					$vbphrase['edit_move_category'] . "\" alt=\"" . $vbphrase['edit_move_category'] . "\"></a>
				<a href=\"javascript:confirmCategoryDelete(" . $record['categoryid'] . ', \'' .
					addslashes_js($vbphrase['confirm_deletion']). "');\"><img src=\"../images/cms/delete_small.png\" style=\"border-style:none\"  alt=\"" .
		...
		". self::getNav($per_page, $categories['count'], $page, 'category', 100) . "
		</div>\n\n\n\n\n</td></tr></table>";
	...
	*********/
	private static function getCategoryEditPanel()
	{
		global $vbphrase;
		$result = "
		<div style=\"position:absolute;width:700px;top:30px;left:50px;height:450px;display:none;background-color:#ffffff;border:3px solid black;\"
		...
		<div style=\"left:0px;top:0px;position:absolute;text-align:left;\"><strong>" .  $vbphrase['category_editor_section_navigator'] /* $vbphrase['edit_category'] */ . "</strong>
		</div><br />&nbsp;
		<div style=\"left:50%;width:50%;top:0px;position:absolute;text-align:right;\">
			<input type=\"button\" value=\"" . $vbphrase['save'] . "\" onclick=\"setCategory('" .
			str_replace('"', '\"', vB_Template_Runtime::escapeJS(htmlspecialchars($vbphrase['need_category_title']))) . "', '" .
			str_replace('"', '\"', vB_Template_Runtime::escapeJS(htmlspecialchars($vbphrase['need_section_or_category']))) . "' );\">
			<input type=\"button\" value=\"" . $vbphrase['close'] .
	...
	<div id=\"section_picker_controls\">
		<div id=\"section_tab\" style=\"font-size:14px;font-weight:bold;padding:2px;padding-bottom:5px;position:absolute;left:5px;top:50px;background-color:#b2b2ba;
		...
		<div class=\"picker_overlay\" id=\"catedit_section_list\" style=\"height:340px;width:330px;left:5px;top:95px;overflow:auto;position:absolute;display:block;\">
	";
	...
	"<img class=\"selected_marker\" id=\"sectchecked_img_$counter\" style=\"display:none\"
	src=\"../images/misc/subscribed.png\" alt=\"" .$vbphrase['you_are_managing_sub_categories_for'] . " " . $section['leaf'] . "\">" .$section['parent'] .
	"<a class=\"section_switch_link\" title=\"". $vbphrase['click_here_to_switch_to']. " " . $section['leaf'] . "\" style=\"font-weight:bold;\" href=\"javascript:setSection($sectionid, '');\">"
	. $section['leaf'] . "</a><br />\n"; ;
	}
	$result .= "
		</div><!--catedit_section_list -->
	</div><!--section_picker_controls -->
	<div id=\"category_selector\" style=\"position:absolute;width:330px;left:350px\">
		<div id=\"category_tab\"  style=\"font-size:14px;font-weight:bold;padding:2px;padding-bottom:5px;position:absolute;left:5px;top:5px;background-color:#b2b2ba;
		...
		<div id=\"catedit_category_list\" style=\"height:345px;width:340px;left:5px;top:50px;overflow:auto;position:absolute;display:block;border-style:solid;border-width:1px 1px 1px 1px;\">
	...
	private static function getSectionEditPanel()
	{
		...
		<div class=\"picker_overlay\" id=\"catedit_section_list\" style=\"height:300px;width:580px;left:10px;top:35px;overflow:auto;position:absolute;font-weight:bold;\">
		" . $vbphrase['enter_section_title'] . "&nbsp;<input type=\"text\" size=\"20\" id=\"section_title\" name=\"section_title\"><br>
		";
		$sections = self::getNodes();
		//Here is the section list
		$counter = 0;
		foreach ($sections as $sectionid => $section)
		{
			...
			$result .= "<input type=\"hidden\" id=\"sectedit_id_$counter\" value=\"$sectionid\">" .
				"<img id=\"sectchecked_img_$counter\" style=\"display:none\"
					src=\"../images/misc/subscribed.png\">" .$section['parent'] .
...
//		}
	...
	***/
	public static function getTimeOffset($userinfo)
	{
	...
	private static function getNodeHeaders()
	{
		...
		$return = "
			<div class=\"tcat\" align=\"right\" style=\"margin:auto;text-align:right;padding:5px;\" id=\"node_mgr_header\" >
	...
	private static function getSectionHeaders($sectionid = 1)
	{
		...
		$return = "
			<div class=\"tcat\" align=\"right\" style=\"margin:auto;padding:5px;\" id=\"node_mgr_header\" >";
	...
	private static function getCategoryHeaders($nodeid)
	{
		...
		*/
				...
				//"<img src=\"../images/cms/add_small.png\" style=\"border:none;\" /></a>".
		...
		switch($vbulletin->GPC['do'])
		{
			...
			case 'delete_section':
				...
				if ($vbulletin->GPC_exists['delete_sectionid'])
					...
					TABLE_PREFIX . "cms_node WHERE new != 1 AND parentnode = " . $vbulletin->GPC['delete_sectionid'])
				...
				$vbulletin->db->query_write("DELETE FROM " .
					TABLE_PREFIX .	"cms_category WHERE parentnode = " . $vbulletin->GPC['delete_sectionid']);
				vB_Cache::instance()->event('sections_updated');
				break;
			...
			case 'publish_nodes':
				...
				if (count($checked))
				{
					...
					vB_Cache::instance()->event('sections_updated');			break;
				}
				vB_Cache::instance()->event('sections_updated');
			...
			case 'move_section':
				...
				{
					$msg = self::moveSection($nodelist, $vbulletin->GPC['sectionid']);
				}
			...
			case 'save_section':
				...
				{
					$msg .= self::moveSection(array($this_id), $vbulletin->GPC['target_sectionid']);
				}
				vB_Cache::instance()->event('section_nav_' . $this_id);
				vB_Cache::instance()->event('sections_updated');
			...
			case 'swap_sections':
				if ($vbulletin->GPC_exists['nodeid'] AND $vbulletin->GPC_exists['nodeid2'])
				{
					$node1 = $vbulletin->GPC['nodeid'];
					$node2 = $vbulletin->GPC['nodeid2'];
			...
			case 'new_section':
				...
				if (!$vbulletin->GPC_exists['sectionid'] AND $vbulletin->GPC_exists['sectionid'])
				{
					$vbulletin->GPC_exists['parentnode'] = $vbulletin->GPC_exists['sectionid'] = 1;
					$vbulletin->GPC['parentnode'] = $vbulletin->GPC['sectionid'] = $vbulletin->GPC['sectionid'];
				}
				if ($vbulletin->GPC_exists['title'] AND $vbulletin->GPC_exists['sectionid']
					and $vbulletin->GPC['sectionid'])
				{
					{
						...
						if ($content = vBCms_Content::create('vBCms', 'Section'))
						{
							...
							$nodedm->set('parentnode', $vbulletin->GPC['sectionid']);
...
/*		$result .= "<tr class=\"tcat\"><td><div class=\"tcat\" style=\"width:100%;margin:auto;text-align:left;padding:5px;font-size:11px\" id=\"node_filters\">
			" . self::getSearchFilters($displayfor)
			. "</div></td></tr>\n" ; */
		$result .= "<tr class=\"tcat\">
				<td colspan=\"10\" align=\"left\" class=\"feature_management_header\" style=\"padding:5px;\">
		...
		if ($record_count = count($nodes))
		{
			...
			<td class=\"thead\">&nbsp;</td>
			<td align=\"left\" width=\"33%\" class=\"thead\"><a href=\"cms_content_admin.php?do=sort&sentfrom=nodes&sortby=title\" target=\"_self\">" . $vbphrase['title'] . "</a></td>
			...
			foreach($nodes as $node)
			{
				...
				$result .= "  <td align=\"left\" class=\"$bgclass\"><a href=\"" . $content_url . "\" target=\"_blank\" >" . self::stripScript($node['title']) 	. "</a></td>\n";
				...
				$result .= "  <td align=\"left\" class=\"$bgclass\">" . $node['username'] . "</td>\n";
		...
		$result .= "</table></td></tr>\n";
		$result .= "</table>";
		$result .= self::getNav($per_page, $total_records, $page, $displayfor, 100);
		global $echoform;
		$echoform = false;
		$result .= "</form>";
		return $result;
	...
	* ***/
	public static function fixNodeLR($sectionid = false, $nodeleft = 0, $sectiontypeid = false, $findmissing = true)
	{
		...
		if ($findmissing)
		{
			...
			if (count($orphans))
			{
				...
				$vbulletin->db->query_write("UPDATE " . TABLE_PREFIX . "cms_node SET parentnode = $sectionid
	...
	public static function listSections($page, $per_page = 10)
	{
		...
		if ($record_count = count($sections))
		{
			...
			$result .= "<tr class=\"tcat\">
					<td class=\"feature_management_header\" style=\"padding:5px;float:left;\"><div style=\"float:left\">
			...
			$result .= "<td class=\"thead\" width=\"20\">#</td>
				<td class=\"thead\" align=\"left\" width=\"400\"><a href=\"cms_content_admin.php?do=sort&sentfrom=section&sortby=config4.value\" target=\"_self\">" . $vbphrase['title'] . "</a></td>
...
/*				<td class=\"thead\">" . $vbphrase['layout'] . "</td>
			...
			foreach($sections as $key => $section)
			{
				...
				{
					$change_display_order_buttons = "<div style=\"float:left; width:32px;\">";
					// dont display up arrow if its already first section in list
					if ($sequence > 2 AND isset($sections[$key-1]))
					{
						$change_display_order_buttons .= "<a style=\"float:left;\" href=\"javascript:swapSections(".$section['nodeid'].", ".$sections[$key-1]['nodeid'].")\"><img src=\"../images/cms/arrow_up.png\" style=\"border-style:none\" /></a>";
					}
					// dont display down arrow is its already last section in list
					if ($sequence < count($sections) AND isset($sections[$key+1]))
					{
						$change_display_order_buttons .= "<a style=\"float:right;\" href=\"javascript:swapSections(".$section['nodeid'].", ".$sections[$key+1]['nodeid'].")\"><img src=\"../images/cms/arrow_down.png\" style=\"border-style:none\" /></a>";
					}
				...
				$result .= "  <td align=\"left\" class=\"$bgclass\" style=\"font-size:80%;width:400px;\"><div class=\"sectionTitleWrapper\" style=\"width:400px;\">
					...
					vb_Types::instance()->getContentTypeID("vBCms_Section") .
						"\" target=\"_self\" >" . vB_Template_Runtime::escapeJS(htmlspecialchars(($section['title']))) 	. "</a>
						<div style=\"float:right\">
						...
						<a href=\"javascript:showSectionEdit('save_section',". (intval($section['parentnode']) ? $section['parentnode'] : '0') . ', ' .
						 $section['nodeid'] . ", '"
					.  vB_Template_Runtime::escapeJS(htmlspecialchars(($section['title']))) . "')\")\"><img src=\"../images/cms/edit_small.png\" style=\"border-style:none\"></a>"
...
/*				$result .= "  <td class=\"$bgclass\" style=\"font-size:80%;\"><select id=\"layout_" . $section['nodeid']. "\" name=\"layout_" . $section['nodeid']. "\"
			...
			$result .= "</div></td></tr>";
			$result .= "</table>\n";
			$result .= self::getNav($per_page, $record_count, $page, 'section', 100);
			global $echoform;
			$echoform = false;
			$result .= "</form>";
	...
	*********************/
	...
	public static function getSections($withcontent = true, $all_nav = false)
	{
		...
		$rst = vB::$vbulletin->db->query_read($sql = "SELECT node.nodeid, node.url, node.parentnode, node.nodeleft, node.noderight,
			node.setpublish, node.publishdate, info.title, node.permissionsfrom, COUNT(child.nodeid) AS children,
			...
			node.setpublish, node.publishdate, info.title, node.permissionsfrom " .
