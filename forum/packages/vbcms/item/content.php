...
class vBCms_Item_Content extends vB_Item_Content
{
	...
	*
	...
	protected function getLoadQuery($required_query)
	{
		...
		if (self::QUERY_BASIC == intval($required_query))
		{
			$sql =
				...
				($this->requireLoad(self::INFO_DEPTH) ?
				" LEFT JOIN " . TABLE_PREFIX . "cms_node AS pdepth ON node.nodeleft BETWEEN pdepth.nodeleft AND pdepth.noderight " : '') . "
				$hook_query_joins
				WHERE ";
		...
		else if (self::QUERY_NAVIGATION == $required_query)
		{
			...
			return
				"SELECT navigation.nodeid, navigation.nodelist, nodeinfo.title
				$hook_query_fields
	...
	protected function writeCache()
	{
		...
		if ($this->cached_data AND $this->loaded_info AND (($this->cached_data & $this->loaded_info) == $this->loaded_info ))
		{
			return true;
		}
		...
		$this->cached_data = $this->cached_data | $this->loaded_info;
		// Write the cache
	...
	*  **/
	...
	public function getHtmlTitle()
	{
		...
		return htmlspecialchars_uni($this->html_title);
	}
	...
	******/
	public function canView()
	{
		...
		//Hidden flag applies to the section. An article is never hidden if we're going directly
		// to it.
		...
		if (in_array($this->permissionsfrom, vB::$vbulletin->userinfo['permissions']['cms']['canedit']))
		{
			return true;
		}

		return (in_array($this->permissionsfrom, vB::$vbulletin->userinfo['permissions']['cms']['canview'])
			AND $this->setpublish AND ($this->publishdate <= TIMENOW));
	}
	...
	public function getParentage()
	{
		...
		if ($rst = vB::$vbulletin->db->query_read("SELECT node.nodeleft, info.title FROM " .
			...
			"cms_node AS myself ON myself.nodeleft
			 BETWEEN node.nodeleft AND node.noderight WHERE myself.nodeid =" . $this->parentnode . "
		...
		if ($this->canPublish())
		{
			...
			$nodelist = vBCms_ContentManager::getNodes(1,
				array('contenttypeid' => 'node2.contenttypeid = ' . vb_Types::instance()->getContentTypeID("vBCms_Section"),
					'privileges' => "node2.permissionsfrom in (" . implode(',', vB::$vbulletin->userinfo['permissions']['cms']['cancreate']) .")"));

			foreach ($nodelist as $nodeid => $value)
			{
				$nodelist[$nodeid]['selected'] = ($sectionid == $nodeid ? 'selected="selected"' : '');
			}
			...
			WHERE nc.nodeid = " . $this->nodeid ))
		{
			while($record = vB::$vbulletin->db->fetch_array($rst))
			{
				$route_info = $record['categoryid'] .
					($record['category'] != '' ? '-' . str_replace(' ', '-', vB_Search_Searchtools::stripHtmlTags($record['category'])) : '');
