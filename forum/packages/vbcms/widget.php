...
abstract class vBCms_Widget
{
	...
	protected function __construct($widget = false)
	{
		...
		{
			throw (new vB_Exception_Widget('No package or contenttype class defined for widget item \'' . get_class($this) . '\''));
		}
	...
	protected function assertWidget()
	{
		if (!$this->widget)
		{
			if (!$this->widgetid)
			{
				throw (new vB_Exception_Widget('No widgetid given to widget handler \'' . get_class($this) . '\' for loading widget'));
			}
		...
		{
			throw (new vB_Exception_Widget('Could not load widget \'' . get_class($this) . '\' with id \'' . htmlspecialchars($this->widgetid) . '\''));
		}
	}
	...
	{
		throw (new vB_Exception_Widget('Config view requested from widget \'' . htmlspecialchars($this->title) . '\' but getConfigView() is undefined'));
	}
	...
	{
		throw (new vB_Exception_Widget('Config Editor view requested from widget \'' . htmlspecialchars($this->title) . '\' but getConfigView() is undefined'));
	}
