 ...
 * @date 		$Date: 2010-01-13 17:30:49 -0600 (Wed, 13 Jan 2010) $
 *
 */
class vB_Attachment_Display_Single_vBCMS_Article extends vB_Attachment_Display_Single
{
	...
	*/
	public function verify_attachment()
	{
		...
		if (isset($this->contentid))
		{
			return vBCMS_Permissions::canView($this->contentid);
		}

		if (vB::$vbulletin->GPC_exists['attachmentid'] AND $this->vbulletininfo['contentid'])
		{
...
class vB_Attachment_Display_Multiple_vBCMS_Article extends vB_Attachment_Display_Multiple
{
	...
	public function process_attachment_template($articleinfo, $showthumbs = false)
	{
		...
		$articleinfo['filename'] = htmlspecialchars_uni($articleinfo['filename']);
		$articleinfo['counter'] = vb_number_format($articleinfo['counter']);
		$articleinfo['size'] = vb_number_format($articleinfo['size'], 1, true);
		$articleinfo['postdate'] = vbdate($this->registry->options['dateformat'], $articleinfo['dateline'], true);
		$articleinfo['posttime'] = vbdate($this->registry->options['timeformat'], $articleinfo['dateline']);
