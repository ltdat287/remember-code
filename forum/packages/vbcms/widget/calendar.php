 ...
 * @version $Revision: 35042 $
 * @since $Date: 2010-01-20 16:30:04 -0600 (Wed, 20 Jan 2010) $
	...
	***/
	private function getPublished($year, $month)
	{
		...
		if (!($articles = vB_Cache::instance()->read($hash, true, false)))
		{
			...
			$start = gmmktime (0, 0, 0, $month, 1, $year) - $offset;

			
			$startdate = getdate($start + $offset);
...
//				}
	...
	public static function getCalendar($year, $month)
	{
			...
			$view->textmonth = date('F', gmmktime(1, 1, 1, $month, 15, $year) );
