 ...
 * @version $Revision: 35220 $
 * @since $Date: 2010-01-29 15:41:43 -0600 (Fri, 29 Jan 2010) $
...
class vBCms_Widget_RecentThreads extends vBCms_Widget
{
	...
	public function getPageView()
	{
		...
		//get the data
		$cache_data = vB_Cache::instance()->read($hashkey, true, false);
		if (!$cache_data)
		{
			$cache_data = $this->getThreads();
			vB_Cache::instance()->write($hashkey,
				   $cache_data, $this->cache_ttl);
		}

		// Create view
		$view = new vBCms_View_Widget($this->config['template_name']);
		$view->threads = $cache_data;
	...
	private function getThreads()
	{
		...
		switch (intval($this->config['threads_type']))
		{
			case 0:
				...
				$datecutoffsql = " AND  CAST(thread.dateline AS UNSIGNED) > $datecut";
				break;
			case 1:
				$ordersql = " thread.lastpost DESC";
				$datecutoffsql = " AND  CAST(thread.lastpost AS UNSIGNED) > $datecut";
				break;
			case 2:
		...
		while ($thread = vB::$vbulletin->db->fetch_array($threads))
		{
			...
			$thread['previewtext'] = $parser->get_preview($thread['message'], $this->default_previewlen, false);
			// get avatar
			if (intval($thread['userid']))
			{
