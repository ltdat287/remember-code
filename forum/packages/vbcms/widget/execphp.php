 ...
 * @version $Revision: 35028 $
 * @since $Date: 2010-01-19 17:17:11 -0600 (Tue, 19 Jan 2010) $
...
class vBCms_Widget_ExecPhp extends vBCms_Widget
{
	...
	public function getConfigView()
	{
		...
		if ((vB::$vbulletin->GPC['do'] == 'config') AND $this->verifyPostId())
		{
			...
			vB_Cache::instance()->event('cms_exec_php' . $this->widget->getId());
	...
	public function getPageView()
	{
		...
		if ($view->output = vB_Cache::instance()->read($hash = $this->getHash($this->widget->getId()), true, false))
		{
			return $view;
		}
		...
		try
		{
			...
			vB_Cache::instance()->write($hash,
			   $view->result_html, $config['cache_ttl'], array('cms_exec_php' . $this->widget->getId()));
		}
		catch(Exception $e)
		{
			$view->output = '';
