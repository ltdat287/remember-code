 ...
 * @version $Id: categorynavall.php 35190 2010-01-28 19:14:49Z ksours $
 * @access public
 */
	...
	public function getPageView()
	{
			...
			))
		{
      	...
      	FROM " . TABLE_PREFIX . "cms_node AS node
			INNER JOIN " . TABLE_PREFIX . "cms_node AS parentnode ON node.nodeleft BETWEEN parentnode.nodeleft AND parentnode.noderight
      	...
      	INNER JOIN " . TABLE_PREFIX . "cms_category AS parent on parent.parentnode = node.nodeid
			INNER JOIN " . TABLE_PREFIX . "cms_category AS cat ON cat.catleft BETWEEN parent.catleft AND parent.catright
			LEFT JOIN " . TABLE_PREFIX . "cms_nodecategory AS nodecat ON nodecat.categoryid = cat.categoryid
			WHERE node.setpublish > 0 AND node.publishdate <= " . TIMENOW . " AND " . vBCMS_Permissions::getPermissionString() . "
		...
		$view->widget_title = $this->content->getTitle();
		$view->nodes = $nodes;
		return $view;
	}
