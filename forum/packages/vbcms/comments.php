 ...
 * @version $Id: comments.php 34955 2010-01-13 23:30:49Z ebrown $
 * @access public
 */
class vBCms_Comments
{
	...
	public function getPageView($nodeid, $target_url)
	{
		...
		$base_url = vB_Router::getCurrentURL();
		...
		if (self::canPostComment($view->threadid, $this_user))
		{
			...
			// if user is guest, include captcha validation and guest username field
	...
	***/
	private static function getComments($nodeid, $userinfo, &$permissions, $associatedthreadid)
	{
		...
		$sql = "SELECT distinct post.postid, post.visible, post.dateline
			...
			WHERE threadid = $associatedthreadid AND parentid != 0 AND visible = 1 ORDER BY post.dateline ASC LIMIT 1000";
	...
	*********/
		...
		$perpage, $target_url, $associatedthreadid )
	{
		...
		$perpage = min(max(5, intval($perpage)), 25);
		//get our results
		...
		while ($post =  vB::$vbulletin->db->fetch_array($posts))
		{
			...
			if (!$post['hascustomavatar'])
			{
				...
				else
				{
					...
					$post['avatarurl'] = $post['avatarpath'] = "./images/misc/unknown.gif";
					$post['avwidth'] = 60;
					$post['avheight'] = 60;
				}
			}
