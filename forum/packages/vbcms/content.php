...
abstract class vBCms_Content extends vB_Content
{
	...
	**/
	...
	protected function populateViewContent(vB_View $view, $viewtype = self::VIEW_PAGE)
	{
		...
		{
//			$view->publishdate = vbdate(vB::$vbulletin->options['dateformat'], $date);
...
//		}
	...
	/**
	 * Associate a new thread with this content.
	 *
	 * @return int								- The thradid of the new thread
	 */
	protected function associateThread()
	{
		if ($id = $this->createAssociatedThread(vB::$vbulletin->options['vbcmsforumid'], $this->content))
		{
			if (! vB::$vbulletin->db->query_write("UPDATE " . TABLE_PREFIX . "cms_nodeinfo SET associatedthreadid = $id
				WHERE nodeid = " . $this->content->getNodeId()))
			{
				throw (vB_Exception_Content('Could not set comments thread for content'));
			}

			//it would be nice to set the thread property so that we can call
			//getAssociatedThreadId() after we call this and get the right response.
			//however setProperty is protected and I don't just want to casually
			//make it public for this.  For now we'll need to make sure that
			//we don't call getAssociatedThreadId() after this and instead
			//use the returned threadid (or the thread object from
			//getAssociatedThread) instead.

			//$this->content->setProperty('associatedthreadid', $id);
		}
	...
	public function getParentNodes($nodeid = null, $parents = null)
	{
		...
		if ($rst = vB::$db->query_read("SELECT node2.nodeid, cms_nodeinfo.title, node2.url
			...
			INNER JOIN " . TABLE_PREFIX . "cms_node AS node2 ON (node.nodeleft between node2.nodeleft AND node2.noderight)
