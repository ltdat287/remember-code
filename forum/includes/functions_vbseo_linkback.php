...
WHERE t_dest_url LIKE \"" . addslashes(preg_replace('|#.*$|', '', $dest_url)) . "%\" AND
t_threadid = \"$threadid\""
);
if ($rq['id'])
{
...
VALUES (\"" . time() . "\", \"" . addslashes($src_url) . "\", \"" . addslashes($dest_url) . "\", \"$pingtype\",
\"$postid\",\"$postcount\",\"$threadid\",\"$page\",\"" . addslashes($title) . "\",\"" . addslashes($snippet) . "\","
. ( ($approve && !$wait) ? 1 : 0) . ", \"$incoming\"," . ($wait?1:0) .", 1)"
);
if ($incoming)
{
vbseo_linkback_approve(0, $threadid);
vbseo_send_notification_pingback($threadid, $postid, $src_url, $title, $snippet, ($approve && !$wait) ? 1 : 0, (VBSEO_PINGBACK_NOTIFY && !$wait) ? 1 : 0);
}
}
...
$qf['t_title'], $qf['t_text'], $approve, VBSEO_PINGBACK_NOTIFY);
}else
