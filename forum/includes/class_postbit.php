...
*
* @package 		vBulletin
* @version		$Revision: 35224 $
* @date 		$Date: 2010-01-29 17:08:31 -0600 (Fri, 29 Jan 2010) $
*
*/
...
class vB_Postbit
{
	...
	*/
	function construct_postbit(&$post)
	{
		...
		$show['last_ad'] = $show['first_ad'] = false;
		if ($post['isfirstshown'])
		{
			$ad_location['ad_showthread_firstpost_start'] = vB_Template::create('ad_showthread_firstpost_start')->render();
		...
		$templater = vB_Template::create($this->template_prefix . $this->templatename);
			if ($vbulletin->products['vbcms'])
			{
				...
				if (vB::$vbulletin->userinfo['permissions']['cms']['canpublish'][0] != -1)
				{
	...
	*/
	function process_edit_info()
	{
		...
		$this->post['historyurl'] = 'posthistory.php?' . $vbulletin->session->vars['sessionurl'] . 'p=' . $this->post['postid'];
	}
