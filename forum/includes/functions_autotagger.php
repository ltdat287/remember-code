...
function DeleteThreadTags($thread)
{
        ...
        $tags = $vbulletin->db->query_read("SELECT
                t.tagid
                FROM " . TABLE_PREFIX . "tagcontent as t
...
function ProcessThread($thread)
{
        ...
        {
                $tagcontent[] = "(". $thread['threadid'] .", ". $tag['tagid'] .", " . $thread['postuserid'] . ", " . $thread['dateline'] . ", 1, 2)";
        }
        ...
        if ($tagcontent)
        {
                $vbulletin->db->query_write("
                        INSERT IGNORE INTO " . TABLE_PREFIX . "tagcontent
                                (contentid, tagid, userid, dateline, autotag, contenttypeid)
                        VALUES
                                " . implode(',', $tagcontent)
                );
        }
