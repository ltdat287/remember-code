			...
			while($criteria = $vbulletin->db->fetch_array($criterion))
			{
				switch($criteria['criteriaid'])
				{
					...
					case "browsing_forum_x":
						$conditional_prefix .= '<vb:if condition="$' . 'forumid == ' . $criteria['condition1'] . '">';
						$conditional_postfix .= "</vb:if>";
						break;
					case "browsing_forum_x_and_children":
						...
						$conditional_prefix .= '<vb:if condition="in_array($' . 'forumid, array(' . $forum['childlist'] . '))">';
						$conditional_postfix .= "</vb:if>";
						break;
					case "style_is_x":
					...
					case "is_date":
						...
						{
							$conditional_prefix .= '<vb:if condition="gmdate(\'d-m-Y\', TIMENOW) == ' . $criteria['condition1'] .'">';
							$conditional_postfix .= "</vb:if>";
						}
						else
						{
							$conditional_prefix .= '<vb:if condition="vbdate(\'d-m-Y\', TIMENOW, false, false) == ' . $criteria['condition1'] . '">';
							$conditional_postfix .= "</vb:if>";
						}
						break;
					case "is_time":
