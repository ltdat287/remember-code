...
class vB_Bootstrap
{
	...
	*/
	public function process_templates()
	{
		...
		// #############################################################################
		// Generate Language/Style Chooser Dropdown
		if (empty($_POST['do']))
		{
			$languagechooserbits = construct_language_options('--', true);
			$show['languagechooser'] = ($languagechooserbits ? true : false);
		}
		else
		{
		...
		if ($vbulletin->options['allowchangestyles'] AND empty($_POST['do']))
		{
			...
			$show['quickchooser'] = ($quickchooserbits ? true : false);
		}
		else
		{
		...
		$ad_location['ad_header_end'] = vB_Template::create('ad_header_end')->render();
		$ad_location['ad_navbar_below'] = vB_Template::create('ad_navbar_below')->render();
		...
		$ad_location['ad_footer_start'] = vB_Template::create('ad_footer_start')->render();
		$ad_location['ad_footer_end'] = vB_Template::create('ad_footer_end')->render();
	...
	*/
	public function check_state()
	{
		...
		if (!$vbulletin->options['bbactive'] AND !in_array(THIS_SCRIPT, array('login', 'css'))
		...
		if (!defined('NONOTICES') AND !empty($vbulletin->noticecache) AND is_array($vbulletin->noticecache))
		{
			...
			foreach (fetch_relevant_notice_ids() AS $_noticeid)
			{
				...
				$notice_html = str_replace(
					array('{musername}', '{username}', '{userid}', '{sessionurl}'),
					array($vbulletin->userinfo['musername'], $vbulletin->userinfo['username'], $vbulletin->userinfo['userid'], $vbulletin->session->vars['sessionurl']),
