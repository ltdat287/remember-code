...
*
* @package 		vBulletin
* @version		$Revision: 34973 $
* @date 		$Date: 2010-01-15 11:41:48 -0600 (Fri, 15 Jan 2010) $
*
*/
class vB_BbCodeParser
{
	...
	*/
	function do_parse($text, $do_html = false, $do_smilies = true, $do_bbcode = true , $do_imgcode = true, $do_nl2br = true, $cachable = false)
	{
		...
		$has_img_tag = ($do_bbcode ? $this->contains_bbcode_img_tags($fulltext) : 0);
				...
				if (!empty($this->attachments["$attachmentid"]))
				{
					...
					switch($attachment['extension'])
					{
						...
						case 'pdf':
							...
							if (($settings OR ($this->registry->options['attachthumbs'] AND $attachment['hasthumbnail'])) AND $this->registry->userinfo['showimages'])
							{
