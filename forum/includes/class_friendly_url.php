...
abstract class vB_Friendly_Url
{
	...
	protected function consume_request_pageinfo($clean = false)
	{
		...
		// If using FRIENDLY_URL_BASIC ignore the first query var as it is the uri
		if (FRIENDLY_URL_BASIC == $this->registry->options['friendlyurl'])
		{
			array_shift($query);
		}
	...
	public function parse_request_uri()
	{
		...
		{
			// Check for REWRITE
			$pat_rewrite = preg_quote($this->rewrite_segment, '#') . '/([^\?]*|$)';
	 ...
	 *
	 * @param string $current_uri				- The current uri to check
	 */
	public function redirect_canonical_url($request_uri)
	{
		...
		if ($canonical AND (self::CANON_STRICT == $this->registry->options['friendlyurl_canonical']))
		{
			if ($request_uri != $canonical_uri)
			{
				...
				$request_uri = to_utf8($request_uri, $this->registry->userinfo['lang_charset']);
