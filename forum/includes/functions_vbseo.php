...
VBSEO_URL_GROUPS_DISCUSSION_PAGE : VBSEO_URL_GROUPS_DISCUSSION, $apars);
unset($apars['gmid']);
unset($apars['do']);
unset($apars['page']);
}
...
{
$newurl = vbseo_group_url($apars['page'] ? 
VBSEO_URL_GROUPS_DISCUSSION_PAGE:
VBSEO_URL_GROUPS_DISCUSSION, $apars);
unset($apars['page']);
unset($apars['do']);
unset($apars['group']);
...
'<br /><div style="z-index:3" class="smallfont" align="center">'.$cp_str.'</div>\\1',
$newtext, 1);
}
}
...
$newtext = preg_replace('#(</body>)#is',
'<br /><div style="z-index:3" class="smallfont" align="center">'.$arc_str.'</div>\\1',
$newtext);
}
}
...
if(VBSEO_REWRITE_MEMBERS || VBSEO_REWRITE_AVATAR || VBSEO_REWRITE_BLOGS)
{
...
if(VBSEO_REWRITE_FORUM && (THIS_SCRIPT == 'forumdisplay'))
{
...
if(VBSEO_REWRITE_GROUPS && $_GET['gmid'])
{
$_GET['page'] = vbseo_gmsg_pagenum($_GET['discussionid'], $_GET['gmid']);
$vbse_rurl = vbseo_group_url(($_GET['page'] > 1) ? 
VBSEO_URL_GROUPS_DISCUSSION_PAGE : VBSEO_URL_GROUPS_DISCUSSION, $_GET);
if($vbse_rurl)
{
...
if($isloggedin_changed)
$more_tracking .= "\n".'virtualTracker._setVar("'.(($bbuserinfo['userid']>0) ? 'member' : 'guest').'");';
