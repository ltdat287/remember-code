...
function &fetch_blog_list($blogids, $attachcount, &$postattach, &$categories, $sqljoin = null, $wheresql = null, $deljoinsql = null, $orderby = "blog.dateline DESC")
{
	...
	$blogs = $vbulletin->db->query_read("
		SELECT
			...
			blog_text.pagetext, blog_text.ipaddress AS blogipaddress, blog_text.allowsmilie,
			user.*,
		...
		" . ($vbulletin->userinfo['userid'] ? "LEFT JOIN " . TABLE_PREFIX . "blog_groupmembership AS gm ON (user.userid = gm.bloguserid AND gm.userid = " . $vbulletin->userinfo['userid'] . ")" : '') . "
