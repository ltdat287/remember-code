...
SELECT t.forumid, t.threadid, " . (VBSEO_GET_THREAD_TITLES ? 't.title, ' : '') . "t.replycount, t.lastposter, t.lastpost, t.prefixid
FROM " . vbseo_tbl_prefix('thread') . " AS t
LEFT JOIN " . vbseo_tbl_prefix('deletionlog') . " AS deletionlog ON(deletionlog.primaryid = t.threadid AND type = 'thread')
...
foreach($ids as $id)
{
if (isset($vbseo_gcache['blog'][$id])) continue;
$lookupids[$id]++;
}
if (empty($lookupids))
return;
