...
*
* @package	vBulletin
* @version	$Revision: 34120 $
* @date		$Date: 2009-12-07 13:29:33 -0600 (Mon, 07 Dec 2009) $
*/
...
abstract class vB_StyleVar
{
	...
	protected function print_background_output()
	{
		...
		$cp = '
			...
			<!--
				function previewBackground()
				{
					...
					backgroundString += document.getElementById("color_0").value;
					'
					/* process the image path */
					.'
					if (document.getElementById("it_stylevar['.$this->stylevarid.'][image]_1").value.indexOf("http://")==-1)
					{
						backgroundString+= " " + document.getElementById("it_stylevar[' . $this->stylevarid . '][image]_1").value.replace("(","(../");
					' /* this is again assuming that images folder is stored in root directory */
					.'
					}
					else
					{
						backgroundString+= " " + document.getElementById("it_stylevar[' . $this->stylevarid . '][image]_1").value;
					}
					backgroundString += " " + document.getElementById("sel_stylevar[' . $this->stylevarid . '][repeat]_2").value;
					backgroundString += " " + document.getElementById("it_stylevar[' . $this->stylevarid .'][x]_4").value + document.getElementById("sel_stylevar[' . $this->stylevarid . '][units]_3").value;
					backgroundString += " " + document.getElementById("it_stylevar[' . $this->stylevarid .'][y]_5").value + document.getElementById("sel_stylevar[' . $this->stylevarid . '][units]_3").value;
					document.getElementById("previewBG").style.background=backgroundString;
				}
			-->
			</script>';
		$cell = "
			<div id=\"previewBG\" style=\"
		...
		$label = '<a href="javascript:previewBackground();">'. $vbphrase['click_here_to_preview'] .' </a>';
		print_label_row($label,$cp.$cell);
	}
}
...
class vB_StyleVar_border extends vB_StyleVar
{
	public function print_editor_form()
	{
		...
		$this->print_units($this->value['units']);
		print_input_row($vbphrase['width'], 'stylevar[' . $this->stylevarid . '][width]', $this->value['width'], true);
		print_input_row($vbphrase['style'], 'stylevar[' . $this->stylevarid . '][style]', $this->value['style'], true);
