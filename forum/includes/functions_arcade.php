...
function build_games()
{
	...
	$sessioncounts = $db->query_read("SELECT arcade_sessions.gameid, COUNT(*) AS sessioncount FROM " . TABLE_PREFIX . "arcade_sessions AS arcade_sessions
	...
	{
		if ($gamesession['sessioncount']!=$gamecache[$gamesession['gameid']]['sessioncount'])
		{
			// Sessioncount mismatch, recalculate high scorer.
			if ($champcheck = $db->query_first("SELECT userid, score FROM " . TABLE_PREFIX . "arcade_sessions AS arcade_sessions WHERE valid=1 AND gameid=$gamesession[gameid] ORDER BY score DESC, finish DESC LIMIT 1"))
			{
