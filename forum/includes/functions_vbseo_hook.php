...
global $onload;
if(!$onload && !strstr($_SERVER['HTTP_REFERER'],$_SERVER['VBSEO_URI']))
{
$onload = "var cpost=document.location.hash;if(cpost){ if(cobj = fetch_object(cpost.substring(1,cpost.length)))cobj.scrollIntoView(true);else {cpostno = cpost.substring(5,cpost.length);if(parseInt(cpostno)>0){document.location.href='showthread.php?p='+cpostno;};} }";
}
...
$vbseo_linkback_uri = VBSEO_REWRITE_THREADS ? vbseo_thread_url($thread['threadid']) : 'showthread.' . VBSEO_VB_EXT .'?t='.$thread['threadid'];
...
$vbseoping['t_text_nohtml'] = htmlspecialchars(strip_tags($vbseoping['t_text']));
$vbseoping['t_title_html'] = htmlspecialchars($vbseoping['t_title']);
$vbseoping['t_dest_url'] = str_replace('&amp;amp;','&amp;',str_replace('&','&amp;',$vbseoping['t_title']));
