...
<p>Có thể tên đăng nhập hoặc mật khẩu trong tập tin <code>wp-config.php</code> không chính xác hoặc WordPress không thể kết nối đến máy chủ cơ sở dữ liệu tại <code>%s</code>. Điều này có thể do máy chủ cơ sở dữ liệu đang không chạy.</p>
<ul>
...
<p>Nếu bạn vẫn không giải quyết được vấn đề này hãy liên hệ với người quản lý máy chủ. Ngoài ra bạn có thể thử tìm thêm thông tin tại <a href=\'http://wordpress.org/support/\'>Diễn đàn hỗ trợ của WordPress</a>.</p>
'/*/WP_I18N_DB_CONN_ERROR*/, $dbhost), 'db_connect_fail');
	...
	function set_prefix($prefix) {
		...
		if ( preg_match('|[^a-z0-9_]|i', $prefix) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Tiền tố của cơ sở dữ liệu không hợp lệ'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<p>WordPress có thể kết nối tới máy chủ cơ sở dữ liệu (tên đăng nhập và mật khẩu đã cho là đúng) nhưng không thể chọn được cơ sở dữ liệu <code>%1$s</code>.</p>
<ul>
...
<p>Nếu bạn không biết cách cài đặt cơ sở dữ liệu, bạn hãy <strong>liên lạc với người quản lý máy chủ</strong>. Ngoài ra bạn có thể thử tìm thêm thông tin tại <a href="http://wordpress.org/support/">Diễn đàn hỗ trợ của WordPress</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
			return;
		}
	}
	...
	function print_error($str = '') {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR_FULL*/'Cơ sở dữ liệu của WordPress gặp lỗi %1$s với truy vấn %2$s yêu cầu bởi %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller);
		else
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR*/'Cơ sở dữ liệu của WordPress gặp lỗi %1$s với truy vấn %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query);
	...
	function get_row($query = null, $output = OBJECT, $y = 0) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Kiểu kết quả phải là một trong những kiểu sau: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
