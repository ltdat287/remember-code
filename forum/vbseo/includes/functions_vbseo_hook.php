...
eval(vbseo_eval_template('vbseo_blog_bmarkentry', '$vbseo_post_bookmarks', 1));
}
$vbseo_post_bookmarks = str_replace('"', '\\"', $vbseo_post_bookmarks);
$bsection_tpl = str_replace('<!--VBSEO_BOOKMARKS-->', $vbseo_post_bookmarks, vbseo_fetch_tpl('vbseo_blog_bmarksection'));
...
vbseo_modify_template34('blog_comment', '#(blogipaddress.*?)(</(?:div|span)>)#s', 
'$1 <a href="'.$permalinkurl.'"><img src="'.vBSEO_Storage::path('fimages').'anchor.png" border="0" alt="$vbphrase[vbseo_permalink]" class="inlineimg" /></a> $2',
0, '<!--PERMALINK_INFO-->');
...
vbseo_modify_template34('memberinfo_visitormessage', '#\$message\[date\].*?</span>.*?</span>#s', 
...
eval('$vbseolinkbackbits .= "' . vbseo_fetch_tpl('vbseo_linkbackbit') . '";');
}
if ($vbseolinkbackbits)
eval('$HTML = "' . vbseo_fetch_tpl('vbseo_linkbacks') . '";');
else
...
eval('print_output("' . vbseo_fetch_tpl('USERCP_SHELL') . '");');
}
break;
case 'misc_start':
...
{
vbseo_insert_code(
"var cpost=document.location.hash.substring(1);var cpost2='';if(cpost){ if(cpost.substring(0,4)=='post')cpost2='post_'+cpost.substring(4);if((cobj = fetch_object(cpost))||(cobj = fetch_object(cpost2))){cobj.scrollIntoView(true);}else {cpostno = cpost.substring(4,cpost.length);if(parseInt(cpostno)>0){location.replace('".$vboptions['bburl2'] . "/showthread.php?p='+cpostno);};} }",
'onload');
}
if (VBSEO_IN_PINGBACK || VBSEO_IN_TRACKBACK || VBSEO_IN_REFBACK || VBSEO_BOOKMARK_THREAD)
{
global $vbseo_bookmarks, $vbseo_linkback_menu, $show, $vbseo_linkback_uri, $thread;
...
vbseo_get_thread_info($threadid);
$excpars = array('t', 'p','page', 'posted', 'viewfull');
...
vbseo_add_canonic_url(VBSEO_REWRITE_THREADS ? vbseo_thread_url($thread['threadid'], vbseo_vb_gpc('page')) : 'showthread.' . VBSEO_VB_EXT .'?t='.$thread['threadid'].(vbseo_vb_gpc('page') ? '&page='.vbseo_vb_gpc('page') : ''));
if (VBSEO_IN_PINGBACK || VBSEO_IN_TRACKBACK || VBSEO_IN_REFBACK)
{
...
$vbseo_post_bookmarks = str_replace('images/vbseo', 'vbseo/resources/images/forum', $vbseo_post_bookmarks);
$search_for = '<!--VBSEO_BOOKMARKS-->';
if (!vbseo_tpl_search($tplpostbit, $search_for))
$search_for = 'editlink';
vbseo_process_template('postbit_bookmarks', array(
'tpl' => $tplpostbit,
'abm' => $vbseo_post_bookmarks,
