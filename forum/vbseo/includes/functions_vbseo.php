...
{
/*
if(($base_script == 'member.'.VBSEO_VB_EXT)||
($base_script == 'group.'.VBSEO_VB_EXT))
{
if(isset($apars['attachmentid']) )
{
$apars['pictureid'] = $apars['attachmentid'];
...
if($newurl = vbseo_cms_url($apars['r'] ? $apars['r'] : $phpslash_append, 'category'))
{
$url_script = $newurl;
unset($apars['r']);
}
...
if(VBSEO_REWRITE_CMS)
if($newurl = vbseo_cms_url($apars['r'] ? $apars['r'] : $phpslash_append, 'content', false, $apars))
{
$url_script = $newurl;
unset($apars['r']);
unset($apars['page']);
...
Header('X-Processed-By: vBSEO '.VBSEO_VERSION2_MORE.' vBSEO');
...
'Search Engine Friendly URLs by vBSEO '.VBSEO_VERSION2_MORE.' &copy;2010, Crawlability, Inc.',
'Content Relevant URLs by vBSEO '.VBSEO_VERSION2_MORE.' &copy;2010, Crawlability, Inc.',
'Search Engine Optimization by vBSEO '.VBSEO_VERSION2_MORE.' &copy;2010, Crawlability, Inc.',
'SEO by vBSEO '.VBSEO_VERSION2_MORE.' &copy;2010, Crawlability, Inc.',
'LinkBacks Enabled by vBSEO '.VBSEO_VERSION2_MORE,
'LinkBacks Enabled by vBSEO '.VBSEO_VERSION2_MORE.' &copy; 2010, Crawlability, Inc.',
);
...
'<a rel="nofollow" href="">'.($clinked ? 'vBSEO-DGT' : $cp_str).'</a>', $cp_str);
else
if($clinked)
...
if(VBSEO_TWEETBOARD && VBSEO_TWEETBOARD_USER && (!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off'))
{
vbseo_insert_code("
...
$more_tracking[] = "['_setCustomVar', 1, 'usertype', '".(($bbuserinfo['userid']>0) ? 'member' : 'guest')."', 1]";
else
...
$more_tracking[] = "['_setCustomVar', 1, 'usergroup', '".($bbuserinfo['usergroupid'].'-'.htmlspecialchars($grouptitle))."', 1]";
}
}
