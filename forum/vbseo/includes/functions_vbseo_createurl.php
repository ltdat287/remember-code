...
return vbseo_album_url('VBSEO_URL_MEMBER_PICTURE_IMG', $apars);
...
$vbseo_gcache['groupsdis'][$apars['discussionid']] = verify_socialdiscussion($apars['discussionid']);
...
$urlformat = VBSEO_URL_CMS_SECTION;
if($_c['contenttypeid'] && !$_c['parentnode'])
$urlformat = VBSEO_URL_CMS_HOME;
if($type == 'category')
$urlformat = $c_page ? VBSEO_URL_CMS_SECTION_PAGE : VBSEO_URL_CMS_SECTION_LIST;
}else
{
...
{
$replace['%section_id%'] = $_c2['tyid'];
$replace['%section_title%'] = vbseo_filter_text($_c2['title']);
}
}
if($c_page)
...
if(strstr($returl, '%'))
$returl = '';
if ($returl && defined('VBSEO_URL_CMS_DOMAIN'))
...
' "' . $db->vbseodb_escape_string($ttl2) . '" order by length('.$whr.') limit 1');
if($queryId)
$mg = $db->vbseodb_fetch_array($queryId);
if(!$mg && $ttl3)
{
$queryId = $db->vbseodb_query($q=$preq.' regexp "' . $db->vbseodb_escape_string($ttl3) . '" order by length('.$whr.') limit 1');
if($queryId)
$mg = $db->vbseodb_fetch_array($queryId);
}
}
