...
$m2 = md5(md5($password));
if($cookie)
$login_defined = md5(md5($login_defined . VBSEO_CONFIG_ID));
else
$password = md5(md5($password));
self::$logged_in = ($password == $login_defined);
...
{
setcookie('vbseocpid', md5(md5(vBSEO_Storage::setting('VBSEO_ADMIN_PASSWORD') . VBSEO_CONFIG_ID)));
return true;
}else
return false;
}
...
public static function check_legacy_files($afiles)
{
$ffiles = array();
foreach($afiles as $f)
{
...
$output = preg_replace('#\{path:(\w+)\}#e', 'vBSEO_Storage::path("$1")', $output);
$output = preg_replace('#\{opt:(\w+)\[(.*?)\]\}#e', 'htmlspecialchars(vBSEO_Storage::setting("$1","$2"))', $output);
$output = preg_replace('#\{opt:(\w+)\}#e', 'vBSEO_Storage::setting("$1")', $output);
$output = preg_replace('#\{lang_esc:([\w\_]+)\}#e', 'addslashes(vBSEO_CP::lang("$1"))', $output);
$output = preg_replace('#\{lang:([\w\_]+)\}#e', 'vBSEO_CP::lang("$1")', $output);
$output = preg_replace('#\{var_esc:([\w\_]+)\}#e', 'htmlspecialchars(\$vars["$1"])', $output);
$output = preg_replace('#\{var:([\w\_]+)\}#e', '\$vars["$1"]', $output);
return $output;
}
...
{
$lng = vBSEO_Storage::setting('VBSEO_CP_LANGUAGE');
if(!$lng)$lng = 'english';
...
self::$lang = array();
foreach($langm as $kl)
self::$lang[$kl[1]] = self::unhtmlentities($kl[2]);
}
...
public static function get_preset_match(&$presets, $type)
{
$pmatch = '';
...
vBSEO_CP::proc_settings($all_settings, true);
vBSEO_CP::save_settings($all_settings);
}
}
...
{
$settings['VBSEO_KEEP_STOPWORDS'] = ($settings['combo_stopwords']>0) ? 1 : 0; 
