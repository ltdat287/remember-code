...
{
$sm = (
(substr(PHP_OS, 0, 3) == 'WIN' AND stristr($_SERVER['SERVER_SOFTWARE'], 'apache') === false) OR (strpos(@php_sapi_name(), 'cgi') !== false AND @!ini_get('cgi.fix_pathinfo')))
? '?' : '/';
...
{
$found_object_ids['pic'][] = $_GET[VBSEO_PICID_URI];
vbseo_get_object_info('pic');
$found_object_ids['album'][] = $_GET['albumid'];
vbseo_get_object_info('album');
vbseo_get_user_info(array($vbseo_gcache['album'][$_GET['albumid']]['userid']));
...
if (VBSEO_REWRITE_CMS && vbseo_check_multi_urls(
array(
...
(isset($_POST) && (isset($_POST['ajax']) || ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) 
&& 
