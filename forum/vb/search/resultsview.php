...
class vb_Search_Resultsview
{
	...
	***/
	private function render_searchbits($page)
	{
		...
		if (!$criteria->is_common() AND count($items_by_type) == 1)
		{
			$options = $type->get_inlinemod_options();

			if ($options)
			{
				$show['inlinemod'] = true;
				$this->template->register('mod_options', $this->render_mod_options($options));
				$this->template->register('mod_type', $type->get_inlinemod_type());
				$this->template->register('mod_action', $type->get_inlinemod_action());
