...
*/
	...
	public function add_userid_filter($userids, $groupuser)
	{
		...
		if (!count($safe_ids))
		{
			...
			$this->add_error('invalidid', $vbphrase['user'], $vbulletin->options['contactuslink']);
			return;
		}
...
//				$this->errors[] = array('searchinvalidterm', htmlspecialchars_uni($word), $minlength);
	...
	/** Sets the keyword search mode to natural or boolean **/
	public function set_naturalmode($natural_mode = true)
	{
		$this->natural_mode = $natural_mode;
	}

	/** Sets the keyword search mode to natural or boolean **/
	public function get_naturalmode()
	{
		return $this->natural_mode;
	}
