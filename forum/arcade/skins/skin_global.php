...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class='maintitle'>
     <tr>
       ...
       <td width="80%" background="<{cat_top_bg}>" class="maintitle"><div align="center"><table  width="100%" cellspacing="0"  id="submenu">
<tr>
<td align="right">
<{CAT_IMG}>&nbsp; <a href='{$ibforums->base_url}act=Arcade'>Arcade</a>
&nbsp;&nbsp;&nbsp;
<border="0" alt="" /><{CAT_IMG}>&nbsp;<a href='{$ibforums->vars['home_url']}'>{$ibforums->vars['home_name']}</a>&nbsp;&nbsp;&nbsp;<!--IBF.RULES-->&nbsp;
 <border="0" alt="" /> <{CAT_IMG}>&nbsp; <a href='{$ibforums->base_url}act=Help'>{$ibforums->lang['tb_help']}</a>
   &nbsp; &nbsp;<border="0" alt="" /><{CAT_IMG}>&nbsp;<a href='{$ibforums->base_url}act=Search&amp;f={$ibforums->input['f']}'>{$ibforums->lang['tb_search']}</a>
   &nbsp; &nbsp;<border="0" alt="" /><{CAT_IMG}>&nbsp;<a href='{$ibforums->base_url}act=Members'>{$ibforums->lang['tb_mlist']}</a>
   &nbsp; &nbsp;<border="0" alt="" /><{CAT_IMG}>&nbsp;<a href='{$ibforums->base_url}act=calendar'>{$ibforums->lang['tb_calendar']}</a>
   <!--IBF.CHATLINK-->
   <!--IBF.TSLLINK-->&nbsp; &nbsp;
 </div>
 </tr>
...
<table width="100%" id="userlinks" cellspacing="2">
   <tr>
    <td align='left'><strong>{$ibforums->lang['logged_in_as']} <a href='./member.php?$session[sessionurl]&amp;u={$ibforums->member['id']}'>{$ibforums->member['name']}</a></strong> ( <a href='{$ibforums->base_url}act=Login&amp;CODE=03'>{$ibforums->lang['log_out']}</a>$ad_link $mod_link $val_link )</td>
    <td align='right'>
...
/*
return <<<EOF
<form action='{$ibforums->vars['board_url']}/login.{$ibforums->vars['php_ext']}' method='post'>
...
<table width="100%" id="userlinks" cellspacing="2">
   <tr>
    <td align='left'>{$ibforums->lang['guest_stuff']} ( <a href='{$ibforums->base_url}act=Login&amp;CODE=00'>{$ibforums->lang['log_in']}</a> | <a href='{$ibforums->base_url}act=Reg&amp;CODE=00'>{$ibforums->lang['register']}</a> )</td>
...
<table width="100%" id="userlinks" cellspacing="2">
   <tr>
    <td align='left'><b>{$ibforums->lang['logged_in_as']} {$ibforums->member['name']}</b> ( <a href='{$ibforums->base_url}act=Login&amp;CODE=03'>{$ibforums->lang['log_out']}</a>$ad_link $mod_link $val_link )</td>
    <td align='right'>
...
<table width="100%" border="0">
        <tr>
            <td align="right">
                        <font size="1"><b>Help keep the ibProArcade.com servers paid for:</b></font><br />
                        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
                                <input type="hidden" name="cmd" value="_xclick" />
                                <input type="hidden" name="business" value="donate@ibproarcade.com" />
                                <input type="hidden" name="item_name" value="ibProArcade Donation" />
                                <input type="hidden" name="no_note" value="1" />
                                <input type="hidden" name="currency_code" value="USD" />
                                <input type="hidden" name="tax" value="0" />
<input type='hidden' name='mid' value='{$ibforums->member[id]}'>
                                <input type="image" src="./style_images/<#IMG_DIR#>/donate.gif" name="submit" alt="Make payments with PayPal - it's fast, free and secure!" />
                        </form>
                </td>
    </tr>
</table>
...
&nbsp;&middot; <b><a href='{$ibforums->base_url}act=dev_files'>Files</a></b>
EOF;
}
...
global $ibforums;
return eval(standard_error($message));
}
