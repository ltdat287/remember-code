...
class db_driver {
   ...
   //--------------------------------------
		...
		$force_oldsql  = false; // if you expect SQL-problems getting avatars set this to TRUE
		...
		if(strpos($the_query, "avatar,avatar_size")) {
			...
			$legacy_query = "SELECT user.userid, user.avatarid, user.avatarrevision, avatar.avatarpath, customavatar.filename, NOT ISNULL(".$avatarkompatibel.") AS hascustom, customavatar.dateline FROM ibf_user AS user LEFT JOIN ibf_avatar AS avatar ON avatar.avatarid = user.avatarid LEFT JOIN ibf_customavatar AS customavatar ON customavatar.userid = user.userid WHERE customavatar.userid=" . $idneeded . " OR user.userid=".$idneeded;
		...
		if (function_exists("mysql_get_client_info"))
		{
			if(mysql_get_client_info() < 4.1)
			{
				...
				$the_query = $legacy_query;
			}
		}
		else
		{
			// could not read mySQL-version, so better be safe
// ###
// ### if you surely HAVE mySQL version > 4.1 you can comment the next line to improve performance!
// ###
			$the_query = $legacy_query;
