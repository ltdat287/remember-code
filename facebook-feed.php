...
class Facebook_Feed {
	...
	function fts_fb_func() {
		$FB_array = $this->FB_array;
		extract($FB_array);
		$custom_name = $posts_displayed;
		$fts_limiter = '5';
		$fts_fb_id = $id;
		$access_token = $this->config['access_token'];
// var_dump(__DIR__.'/projectfiles/'); die();
		//Error Check
		if (!$fts_fb_id){
		...
		$des_cache = __DIR__ . '/cache/FB_des_cache-'.$fts_fb_id.'-num'.$fts_limiter.'.json';
		//Url to get feed info
		$data_cache = __DIR__ . '/cache/FB_data_cache-'.$fts_fb_id.'-num'.$fts_limiter.'.json';
		//Urls to get comments and likes info
		$comment_count_data_cache = __DIR__ . '/cache/FB_cc_cache-'.$fts_fb_id.'-num'.$fts_limiter.'.json';
		$like_count_data_cache = __DIR__ . '/cache/FB_like_cache-'.$fts_fb_id.'-num'.$fts_limiter.'.json';
		...
		foreach($data->data as $d) {
			...
			$FBtype = (! empty($d->type)) ? $d->type : '';
			$FBmessage = (! empty($d->message)) ? $d->message : '';
			$FBpicture = (! empty($d->picture)) ? $d->picture : '';
			$FBlink = (! empty($d->link)) ? $d->link : '';
			$FBname = (! empty($d->name)) ? $d->name : '';
			$FBcaption = (! empty($d->caption)) ? $d->caption : '';
			$FBdescription = (! empty($d->description)) ? $d->description : '';
			$FBstory = (! empty($d->story)) ? $d->story : '';
			$FBicon = (! empty($d->icon)) ? $d->icon : '';
			$FBby = (! empty($d->properties->text)) ? $d->properties->text : '';
			$FBbylink = (! empty($d->properties->href)) ? $d->properties->href : '';
			$FBpost_id = (! empty($d->id)) ? $d->id : '';
			$FBpost_share_count = (! empty($d->shares->count)) ? $d->shares->count : '';
			$FBpost_like_count_array = (! empty($d->likes->data)) ? $d->likes->data : '';
			$FBpost_comments_count_array = (! empty($d->comments->data)) ? $d->comments->data : '';
			$FBpost_object_id = (! empty($d->object_id)) ? $d->object_id : '';

			$FBfromName = (! empty($d->from->name)) ? $d->from->name : '';
			$FBstory = (! empty($d->story)) ? $d->story : '';
